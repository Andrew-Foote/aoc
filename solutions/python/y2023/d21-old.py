...
def step(m: sparse.csr_array, count: int) -> sparse.csr_array:
	...
	# ok, latest version of scipy DOES have a sparse.linalg.matrix_power
	# function, but if we use it, we get the wrong answer! weird
...
def ten_step_count(ip: str) -> int: return step_count(ip, 10)
def fifty_step_count(ip: str) -> int: return step_count(ip, 50)
def hundred_step_count(ip: str) -> int: return step_count(ip, 100)
def five_hundred_step_count(ip: str) -> int: return step_count(ip, 500)
def thousand_step_count(ip: str) -> int: return step_count(ip, 1000)
def five_thousand_step_count(ip: str) -> int: return step_count(ip, 5000)
...
def p2(ip: str) -> int:
	...
	# i think we just need to compute the "reachable set"
	# this is obtainable by searching the graph and not returning to any visited
	# nodes and implementing the limit
	# then once we've got the reachable set we count
		# well, we'll have all the ones on the "outer" part reachable in N steps
		# we can check how many nodes on the "inside" are adjacnet
