...
def poss_group_locs(
    row: UnknownRow, size: int, offset: int, uncovered: frozenset[int], is_last: bool=False
    ...
    result: list[tuple[int, frozenset[int]]] = []
    ...
    if is_last:
        i_s = sum((tuple(range(i - size + 1, i + size)) for i in uncovered), start=())
        print(uncovered, i_s)
        i_s = frozenset(i for i in i_s if offset <= i < rowlen - size + 1)
        ...
        for i in i_s:
            ...
            result.append((i, uncovered - frozenset(span)))
...
def pgls(
    row: UnknownRow, sizes: tuple[int, ...],
    uncovered: frozenset[int] | None=None
) -> list[tuple[int, ...]]:

    if uncovered is None:
        uncovered = frozenset(i for i, state in enumerate(row.items) if state == SpringState.DAMAGED)
    ...
    for loc, new_uncovered in poss_group_locs(row, size, 0, uncovered, is_last=not sizes):
        offset = loc + size + 1
        new_uncovered = frozenset(i - offset for i in new_uncovered if i >= offset)
        ...
        for remaining_locs in pgls(new_row, sizes, new_uncovered):
...
def poss_group_loc_sets(
    ...
    assert offset == 0 # offset is obsolete now
    return pgls(row, sizes, uncovered)
    ...
    for loc, new_uncovered in poss_group_locs(row, size, offset, uncovered, is_last=not sizes):
        new_offset = loc + size + 1

        for remaining_locs in poss_group_loc_sets(row, sizes, new_offset, new_uncovered):
